...
export default function(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    renderCall(args: { path?: string }, theme: any) {
      ...
      return new Text(
        `${theme.fg("toolTitle", theme.bold("diagnostics"))} ${theme.fg("accent", displayPath)}`,
        0,
        0,
      )
    },
    renderResult(
      ...
      if (context.isError) {
        return new Text(theme.fg("error", output || "Failed"), 0, 0)
      }

      if (!options.expanded) {
        if (output === "No diagnostics found.") {
          return new Text(theme.fg("muted", "No issues"), 0, 0)
        }
        const lines = output.split("\n").filter((l) => l.startsWith("  "))
        const errors = lines.filter((l) => l.includes("Error")).length
        const warnings = lines.filter((l) => l.includes("Warning")).length
        const parts: string[] = []
        if (errors > 0) parts.push(theme.fg("error", `${errors} errors`))
        if (warnings > 0) parts.push(theme.fg("warning", `${warnings} warnings`))
        return new Text(
          parts.length > 0 ? parts.join(theme.fg("muted", ", ")) : theme.fg("muted", "No issues"),
          0,
          0,
        )
      }
